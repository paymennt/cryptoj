...
public class ExtendedPublicKey {
	 ...
	 * @return
	 */
	public static ExtendedPublicKey fromPrivateKey(byte[] key, long depth, String fingerprint, BigInteger childNumber,
			String prefix) {
	...
	public PublicKey toPublicKey() {
		byte[] keyBytes = Arrays.copyOfRange(key, 0, 33);
	...
	public String serialize() {
		byte[] keyBytes = Arrays.copyOfRange(key, 0, 33);
		byte[] chainCode = Arrays.copyOfRange(key, 33, key.length);
		ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
